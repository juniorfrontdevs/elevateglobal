...
treasureButton.addEventListener('click',()=>{bonusModal.hidden=false;});
closeBonus.addEventListener('click',()=>{bonusModal.hidden=true;});
bonusModal.addEventListener('click',(event)=>{if(event.target===bonusModal) bonusModal.hidden=true;});
document.addEventListener('keydown',(event)=>{if(event.key==='Escape'&&!bonusModal.hidden) bonusModal.hidden=true;});
window.addEventListener('scroll',()=>{backToTop.classList.toggle('show',window.scrollY>280);});
